...
// game just holds game state and calls functions for eg starting a game
...
public func startGame<Question, Answer: Equatable, R: Router>(
    ...
    let flow = Flow(questions: questions, router: router, scoring: { answers in scoring(answers, correctAnswers: correctAnswers) })
    flow.start()
